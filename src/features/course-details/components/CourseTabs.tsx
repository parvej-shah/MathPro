...
export default function CourseTabs({ activeTab, onTabChange }: CourseTabsProps) {
    const getTabClass = (isActive: boolean) => {
        return `${
            isActive
                ? 'text-primary border-2 border-primary bg-gradient-to-r from-primary/10 to-primary/5 font-semibold shadow-lg shadow-primary/20'
                : 'text-muted-foreground border border-transparent hover:text-primary hover:border-primary/30 hover:bg-primary/5'
