...
const RankingCard: React.FC<RankingCardProps> = ({ courseId, loading = false }) => {
      ...
      >
        <div className="animate-pulse">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 items-center">
            ...
            <div className="h-6 bg-white/20 rounded-full w-24 sm:justify-self-end"></div>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="bg-card backdrop-blur-lg border border-border rounded-2xl p-4">
    ...
    <Link href={`/ranking?course=${courseId}`}>
      ...
      >
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 items-center text-white">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="bg-white/20 p-1.5 sm:p-2 rounded-full group-hover:bg-white/30 transition-colors">
              <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
            </div>
            <div>
              <div className="text-white/80 text-xs sm:text-sm font-medium whitespace-nowrap">
          ...
          {myData?.cf_handle && (
            <div className="col-span-2 sm:col-span-1 flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full w-fit sm:ml-auto">
